...
const ListNote = ({ datas, title, archived = false, onDelete, onArchived, searchFilter }) => {
  ...
  return (
    <section className="notes-arc" id={title}>
      {datas.length !== 0 && searchValue.length !== 0 ? (
        <div className="card-wrapper">
          {searchFilter === ''
            ? datas.map((data, idx) => <ItemNote title={data.title} body={data.body} id={data.id} archived={data.archived} createdAt={data.createdAt} onDelete={onDelete} onArchived={onArchived} key={idx} />)
